  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(error.message || "Failed to create student payment");
      }
      toast.success("Student payment added!");
      (e.target as HTMLFormElement).reset();
    } catch (error: any) {
      toast.error(error.message || "Failed to create student payment");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative z-10 min-h-screen px-6 py-8">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">Add Student Payment</h2>
            <p className="text-gray-300">Record a new student payment transaction</p>
          </div>
          ...
          <div className="bg-white bg-opacity-10 backdrop-blur-xl rounded-3xl p-8 border border-white border-opacity-20 shadow-xl">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  ...
                  <input 
                    ...
                    required 
                    type="tel"
                    className="w-full px-4 py-3 bg-white bg-opacity-10 backdrop-blur-lg border border-white border-opacity-20 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent transition-all duration-300"
                ...
                <div>
                  <label className="block text-sm font-medium text-white mb-2">Paid By (optional)</label>
                  <input 
                    name="paidBy" 
                    className="w-full px-4 py-3 bg-white bg-opacity-10 backdrop-blur-lg border border-white border-opacity-20 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent transition-all duration-300"
                ...
                <div>
                  ...
                  <input 
                    ...
                    type="date" 
                    required 
                    className="w-full px-4 py-3 bg-white bg-opacity-10 backdrop-blur-lg border border-white border-opacity-20 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent transition-all duration-300"
                  />
                </div>
                <div>
              ...
              <button 
                type="submit" 
                disabled={loading} 
                className="w-full bg-gradient-to-r from-pink-500 to-rose-600 text-white py-4 rounded-xl font-semibold hover:shadow-lg transition-all duration-300 transform hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading ? (
