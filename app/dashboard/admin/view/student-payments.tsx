...
const StudentPaymentsView = () => {
  ...
  const [form, setForm] = useState({ studentName: '', amount: '', date: '' });
  ...
  const handleEdit = (payment: StudentPayment) => {
    setSelected(payment);
    setForm({ studentName: payment.studentName, amount: String(payment.amount), date: payment.date });
    setShowModal(true);
  };
  ...
  return (
    <div className="p-4">
      <h2 className="text-xl font-bold mb-4">Student Payments</h2>
      <table className="min-w-full border">
        ...
        <tbody>
          {payments.map((p) => (
            <tr key={p._id}>
              <td className="border px-2 py-1">{p.studentName}</td>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-30 z-50">
          <div className="bg-white p-6 rounded shadow-lg min-w-[300px]">
            ...
            <form onSubmit={handleSubmit}>
              <input className="border p-1 mb-2 w-full" name="studentName" value={form.studentName} onChange={handleChange} placeholder="Student Name" required />
